...
const ReceiptScanner = ({ onExtracted, onAddExpense, categories = [], paymentTypes = [], paidByOptions = [] }) => {
  ...
  const scanReceipt = async () => {
    ...
    try {
      // All API calls go through the relay server — key never in browser or git
      const res = await fetch('http://localhost:5176/scan-receipt', {
        method: 'POST',
      ...
      const relayJson = await res.json();
      if (!relayJson.ok || !relayJson.data) throw new Error(relayJson.error || 'No data from relay');
      const parsed = relayJson.data;
