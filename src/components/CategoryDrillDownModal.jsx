...
import { X, TrendingUp, TrendingDown, Calendar, DollarSign, CreditCard, User, Tag } from 'lucide-react';
...
const CategoryDrillDownModal = ({ isOpen, onClose, category, expenses, filterYear }) => {
  ...
  const byMonth = useMemo(() => {
    ...
    return Object.entries(map)
      .sort((a, b) => MONTHS_ORDER.indexOf(a[0]) - MONTHS_ORDER.indexOf(b[0]));
  }, [paid]);
                ...
                {byMonth.map(([month, val]) => (
                  <div key={month} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 bg-white/8 rounded-full h-5 overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-violet-500 to-purple-400 rounded-full flex items-center justify-end pr-2 transition-all"
                        style={{ width: `${(val / maxMonthVal) * 100}%`, minWidth: 40 }}
                      >
                        <span className="text-white text-[10px] font-bold whitespace-nowrap">${val.toFixed(0)}</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="overflow-x-auto rounded-xl border border-white/10">
            <table className="w-full text-sm min-w-max">
              ...
              <tbody>
                {sorted.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-8 text-purple-400 text-sm">No transactions found</td>
                  </tr>
                ) : sorted.map(e => (
                  <tr key={e.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-2 px-3 whitespace-nowrap">
                      ...
                      }`}>{e.type === 'Recurring' ? 'Rec' : 'Non-Rec'}</span>
                    </td>
